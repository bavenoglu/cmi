		...
		if (toEndPoints.size() == 1) {
			for (EndPoint ep : toEndPoints) {
				...
				if (ep.getProtocolType().compareTo(ProtocolType.HTTP) == 0) {
					toToURI = ep.getWebURI() + "?bridgeEndpoint=true";
...
//		pdService.add(toMock);
		...
		for (final ServiceRequest sr : serviceRequests) {
		...
		for (ServiceRequest sr : serviceRequests) {
			...
			System.out.println("direct:" + sr.getServiceRequestID());
	...
	public void connect(CamelContext context, RedissonClient redisson) throws Exception {
		...
		System.out.println("Device Connection Started:" + milliSeconds);
		this.setIsConnected(true);
		dataSending(context);
		resultReceiving(context, redisson);
		milliSeconds=clock.millis();
		System.out.println("Device Connection Ended:" + milliSeconds);
	}
